...
class DRFSSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, features,deconv, head, num_classes):
        super(DRFSSD, self).__init__()
        self.phase = phase
    ...
    def forward(self, x):
        ...
        x = transformed[0]
        for k, v in enumerate(self.deconv):
            upsize = (transformed[k + 1].size()[2], transformed[k + 1].size()[3])
            x = v(x, upsize)
            x = torch.cat((transformed[k + 1], x), 1)
        x = self.L2Norm(x)
        ...
        for i in range(len(rainbow)):
            ...
            for j in range(i-1,-1,-1):
                t = nn.AdaptiveAvgPool2d((x.size()[-2],x.size()[-1]))(rainbow[j])
                x = torch.cat((t,x),1)

            for j in range(i+1,len(rainbow)):
                t = F.upsample(rainbow[j], size=(x.size()[-2],x.size()[-1]), mode='bilinear')
                x = torch.cat((x,t),1)
...
def add_extras(base, feature_layer, mbox, num_classes, version):
    ...
    for index in range(len(feature_transform_layers)-1):
        deconv_layers.append(BasicDeConv((index+1)*feature_transform_channel,(index+1)*feature_transform_channel,kernel_size=3,stride=2,padding=1))

    rainbow_channels = sum(feature_layer[1][1])
    ...
    for layer, depth, box in zip(feature_layer[1][0], feature_layer[1][1], mbox):
        ...
        loc_layers += [nn.Conv2d(rainbow_channels, box * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(rainbow_channels, box * num_classes, kernel_size=3, padding=1)]
    return base, extra_layers, (feature_transform_layers, pyramid_feature_layers), (loc_layers, conf_layers), deconv_layers
...
def build_drfssd(phase, size=300, num_classes=21):
    ...
    if size != 300:
        ...
        return
    base_, extras_, features_,  head_, deconv_ = add_extras(vgg(str(size), 3), feature_layer, mbox, num_classes, "fssd")

    return DRFSSD(phase, size, base_, extras_, features_, deconv_, head_, num_classes)
